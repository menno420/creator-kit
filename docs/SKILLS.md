...
The registered skill set for creator-kit: every recurring action that
...
| `continuation-prompt` | Carry a session into a fresh one — verify state at HEAD, commit what belongs in the repo, and emit a paste-ready prompt that transfers intent without narrowing it. | `read`, `edit`, `run` | `git fetch origin main && git log -1 --oneline origin/main`<br>`<verify_command>` |
